...
Pager* pager_open(const char* filename) {
    HANDLE hFile = CreateFile(
        ...
        OPEN_ALWAYS,              // Open if exists, create if doesn't exist
    ...
    pager->file_handle = hFile;
    pager->file_length = file_size.QuadPart;

    pager->num_pages = (file_size.QuadPart/ PAGE_SIZE);

  if (file_size.QuadPart % PAGE_SIZE != 0) {
...
void pager_flush(Pager* pager, uint32_t page_num) {
  ...
  BOOL write_result = WriteFile(
    ...
    PAGE_SIZE,                                 // Number of bytes to write
  ...
  if (pager->pages[page_num] == NULL) {
    ...
    if (page_num <= num_pages) {
      ...
      if (SetFilePointerEx(pager->file_handle, offset, NULL, FILE_BEGIN) == 0) {
        DWORD error = GetLastError();
        printf("Error seeking in file: %lu\n", error);
        exit(EXIT_FAILURE);
      }

      DWORD bytes_read;
      if (ReadFile(pager->file_handle, page, PAGE_SIZE, &bytes_read, NULL) == 0) {
        DWORD error = GetLastError();
      ...
      if (bytes_read < PAGE_SIZE) {
          // Clear the rest of the page if we read a partial page
...
void internal_node_split_and_insert(Table *table, uint32_t parent_page_num, uint32_t child_page_num); // function declaration
...
void db_close(Table* table) {
  ...
  int result = fclose(pager->file_handle);
  if (result == -1) {
